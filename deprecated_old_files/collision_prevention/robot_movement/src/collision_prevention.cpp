...
geometry_msgs::Twist collision_prevention::getVelMsg()
{
    ...
    if (this->previous_decision == IDLE) {
        ...
        int min_wish_index = ceil((wish_angle_rad + this->MIN_FRONT_ANGLE_RAD - this->ANGLE_MIN) / this->ANGLE_INCREMENT);
        int max_wish_index = floor((wish_angle_rad + this->MAX_FRONT_ANGLE_RAD - this->ANGLE_MIN) / this->ANGLE_INCREMENT);
        int mid_wish_index = ceil((min_wish_index + max_wish_index) / 2);

        int min_index = ceil((wish_angle_rad + this->MIN_SCAN_ANGLE_RAD - this->scan->angle_min) / this->ANGLE_INCREMENT);
        int max_index = floor((wish_angle_rad + this->MAX_SCAN_ANGLE_RAD - this->scan->angle_min) / this->ANGLE_INCREMENT);

        int min_left_side_index = max_wish_index + 1;
        int max_left_side_index = min_left_side_index + floor(this->SIDE_AREA_ANGLE_RAD / this->ANGLE_INCREMENT);
        int max_right_side_index = min_wish_index - 1;
        int min_right_side_index = max_right_side_index - floor(this->SIDE_AREA_ANGLE_RAD / this->ANGLE_INCREMENT);
